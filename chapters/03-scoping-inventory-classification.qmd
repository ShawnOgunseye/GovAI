...
Before the disputes, one boundary worked through in full, because the eight grounds are easier to state than to apply. Take the list of dependencies as given here; section 3.3 sets out how to generate it.
...
## 3.3 Tracing the boundary

Section 3.2 gives a test. It does not give a list to run the test against, and a test with nothing to test is not usable. Holding a component up against the eight grounds only works once someone has named the component, and the failure in practice is not that people apply the test wrongly. It is that they never think of the dependency at all.

Naming them is a tracing problem, and the trace starts with the use rather than the product. Who uses this capability, for what purpose, to make which decision or take which action, and affecting whom? A product has no boundary. A deployment does, because a deployment has a path running through it, from the moment data is created to the moment a person feels a consequence. Follow that path in four passes.
...
Calloway's payroll system is the clean negative case. It holds employee data, it sits in the same infrastructure, and it is outside the hiring boundary, because nothing in it reaches the screening decision. That holds until the day someone proposes using payroll history as a feature, at which point payroll moves inside and the boundary has changed without a line of the screening system's code being touched.
