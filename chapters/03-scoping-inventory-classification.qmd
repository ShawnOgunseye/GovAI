...
**What turns the result into an outcome.** This is the pass most often skipped, and it is where the consequence actually happens. A score has no effect on anybody until something acts on it. Include the dashboard or report the score arrives in, the person who reads it, the review procedure that person is meant to follow, any automated action that fires without a person, the message sent to the affected individual, and the escalation route for the cases that do not fit. A system scoped to the model and stopped there governs a number, not an outcome.
...
The four passes surface components, processes, and people, and the last of those needs separating out before the boundary is recorded. Two further points follow from what section 3.2 established, and both decide what the boundary contains rather than how wide it is.
...
| Affects people who cannot know it ran | **Population** | No detection means no complaint, so nothing else will surface the failure |
